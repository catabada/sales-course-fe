...
function AdminCourse() {
    ...
    const columns = [
        ...
        },
        {
            name: 'Giá gốc',
            selector: row =>
                Intl.NumberFormat('vi-VN', {style: 'currency', currency: 'VND'}).format(row.oldPrice),
            width: '120px'
        },
        {
            name: 'Giá bán',
    ...
    return <Box sx={{height: '80vh', width: '100%'}}>
        <Box sx={{height: '10vh', display: 'flex', alignItems: 'center'}}>
            <Typography variant="h5" sx={{p: 1, fontWeight: 'bold', fontSize: '2rem'}}>Quản lý khoá học</Typography>
            <CourseAdd/>
            <CourseTrash/>
        </Box>
