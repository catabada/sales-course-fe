...
function SearchItem({ course }) {
    return (
        <Box component={'li'}>
            <Box
                component={Link}
                to={`/course/${course.codeName}`}
                sx={{ display: 'flex', textDecoration: 'none', alignItems: 'center' }}
            >
                <Avatar
                    src={`/images/${course.image}`}
                    alt="course-img"
                    sx={{ height: 70, width: 70, marginRight: '1rem' }}
                />
                    ...
                    <Box className={cx('course-item-price')}>
                        {course.price === 0 ? (
                            ...
                            <>
                                ...
                                <Typography variant="body1" className={cx('price', 'old-price')}>
                                    {Intl.NumberFormat('vi-VN', {
                                        style: 'currency',
                                        currency: 'VND',
                                    }).format(course.price)}
                    ...
                    <div className={cx('content-wrapper')}>
                        ...
                        <ul className={cx('course-list')}>
                            {coursesSearch.map((course, index) => {
                                if (index <= 2) return <SearchItem key={index} course={course} />;
                            })}
                        </ul>
                    </div>
                </div>
